...
import type { AthleteInsight } from '../../types';

export const AthleteInsightsPanel: React.FC = () => {
  const { athleteInsights, currentProfile } = useApp();
  const [dbInsights, setDbInsights] = useState<AthleteInsight[]>([]);
  const coachId = currentProfile?.role === 'coach' ? currentProfile.profile.id : null;

  useEffect(() => {
    if (!coachId) return;
    let isMounted = true;
    void fetchStudentLogs(coachId).then((logs) => {
      if (!isMounted) return;
      const mapped: AthleteInsight[] = logs.map((log) => ({
        id: log.id,
        athleteId: log.athleteId,
        athleteName: 'Athlete Check-In',
        mood: log.mood || 'Check-in',
        summary: `Metric: ${log.metricType} (${log.flag.toUpperCase()})`,
        note: log.notes || '',
        timestamp: log.loggedAt,
        source: log.metricType === 'wellbeing_checkin' ? 'wellbeing' : 'progress',
      }));
  ...
  const allInsights = [...dbInsights, ...athleteInsights];
  const uniqueInsights = Array.from(new Map(allInsights.map((i) => [i.id, i])).values())
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex items-center gap-2 rounded-2xl border border-brand-border bg-white/5 p-3 text-[11px] text-brand-muted">
        ...
        <span>Connected to live Supabase progress logs & wellbeing check-ins.</span>
      </div>
    </motion.div>
  );
};
